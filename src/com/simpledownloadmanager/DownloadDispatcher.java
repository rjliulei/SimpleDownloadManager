...
public class DownloadDispatcher extends Thread {
		...
		while (true) {
			try {
				...
				if (mQuit) {
					if (mRequest != null) {
						mRequest.finish();
						updateDownloadFailed(DownloadManager.ERROR_PAUSED_TO_EXIT, "DownloadManager exit");
						mTimer.cancel();
					}
					return;
				}
	...
	private void executeDownload(String downloadUrl) {
		...
		try {
			...
			conn.setRequestProperty("Range", "bytes=" + new File(mRequest.getTempURI().toString()).length() + "-");

			// Status Connecting is set here before
			// urlConnection is trying to connect to destination.
			mRequest.setDownloadState(DownloadManager.STATUS_CONNECTING, "");
			...
			case HTTP_PARTIAL:
				...
				if (readResponseHeaders(conn) == 1) {
					...
					updateDownloadFailed(DownloadManager.ERROR_DOWNLOAD_SIZE_UNKNOWN,
			...
			case HTTP_INTERNAL_ERROR:
				...
				break;
			default:
				updateDownloadFailed(DownloadManager.ERROR_UNHANDLED_HTTP_CODE, "Unhandled HTTP response:"
						+ responseCode + " message:" + conn.getResponseMessage());
				break;
			}
	...
	private void transferData(HttpURLConnection conn) {
		...
		try {
			...
			try {
				out = new RandomAccessFile(mRequest.getTempURI().toString(), "rwd");
	...
	private void transferData(InputStream in, RandomAccessFile out) {
		...
		mCurrentBytes = 0;
		preSecondBytes = 0;
		mRequest.setDownloadState(DownloadManager.STATUS_RUNNING, null);
		...
		for (;;) {
			...
			if (mRequest.isCanceled()) {
				Log.v(TAG,
						"Stopping the download as Download Request is cancelled for Downloaded Id "
			...
			if (mContentLength != -1 && mContentLength > 0) {
				...
				updateDownloadProgress(progress, mCurrentBytes, speed);
			}
			...
			if ((System.currentTimeMillis() - preTime) >= 1000) {
				...
				speed = (int) ((mCurrentBytes - preSecondBytes) / (System.currentTimeMillis() - preTime)) * 1024;

				preTime = System.currentTimeMillis();
				preSecondBytes = mCurrentBytes;
			}
	...
	private int readResponseHeaders(HttpURLConnection conn) {
		...
		if (transferEncoding == null) {
			...
			mRequest.saveFileSize(mContentLength);
		} else {
			Log.v(TAG,
	...
	private void attemptRetryOnTimeOutException() {
		mRequest.setDownloadState(DownloadManager.STATUS_RETRYING, "");

		final RetryPolicy retryPolicy = mRequest.getRetryPolicy();
		updateDownloadFailed(DownloadManager.ERROR_RETRYING, retryPolicy.getCurrentTimeout() + "秒后重试");
		try {
			retryPolicy.retry();
	...
	private void cleanupDestination() {
		Log.d(TAG, "cleanupDestination() deleting " + mRequest.getTempURI().toString());
		File destinationFile = new File(mRequest.getTempURI().toString());
	...
	public void updateDownloadComplete() {
		...
		new File(mRequest.getTempURI().getPath().toString()).renameTo(new File(mRequest.getDestinationURI().getPath()
				.toString()));
	...
	public void updateDownloadFailed(int errorCode, String errorMsg) {
		...
		case DownloadManager.ERROR_PAUSED_TO_EXIT:
			mRequest.setDownloadState(DownloadManager.STATUS_PAUSED, errorMsg);
			break;
		case DownloadManager.ERROR_DOWNLOAD_CANCELLED:
			mRequest.setDownloadState(DownloadManager.STATUS_FAILED, errorMsg);
			mRequest.deleteInfoInDB();
			cleanupDestination();
			break;
		default:
			mRequest.setDownloadState(DownloadManager.STATUS_FAILED, errorMsg);
	...
	public void updateDownloadProgress(int progress, long downloadedBytes, int speed) {
		if (mRequest.getDownloadListener() != null) {
			mDelivery.postProgressUpdate(mRequest, mContentLength, downloadedBytes, progress, speed);
		}
	}
}
