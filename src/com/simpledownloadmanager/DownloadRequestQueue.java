...
public class DownloadRequestQueue {
	...
	class CallBackDelivery {
		...
		public void postProgressUpdate(final DownloadRequest request, final long totalBytes, final long downloadedBytes, final int progress, final int speed) {
			mCallBackExecutor.execute(new Runnable() {
				public void run() {
					request.getDownloadListener().onProgress(request.getDownloadId(), totalBytes, downloadedBytes, progress, speed);
				}
			});
		}
	}
	...
	public DownloadRequestQueue(int threadPoolSize) {
		...
		} else if(threadPoolSize > MAX_THREAD_POOL_SIZE){
			mDownloadDispatchers = new DownloadDispatcher[MAX_THREAD_POOL_SIZE];
		}else {
			mDownloadDispatchers = new DownloadDispatcher[DEFAULT_DOWNLOAD_THREAD_POOL_SIZE];
		}
	}
